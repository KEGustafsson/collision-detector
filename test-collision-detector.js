 ...
 * Tests the new implementation (index-new.js)
 */

const assert = require('assert');

// Load the new implementation and extract functions for testing
// We'll need to modify index-new.js slightly to export test functions
// For now, we'll copy the pure functions here for testing
...
// ============================================================================
// PURE FUNCTIONS (copied from implementation for testing)
// ============================================================================
...
test('1.3 Haversine vs Equirectangular: short distance', () => {
	const from = { latitude: 60.0, longitude: 24.0 };
	const to = { latitude: 60.01, longitude: 24.01 };
	const distEq = equirectangularDistance(from, to);
	const distHav = haversineDistance(from, to);
	assertApprox(distEq, distHav, 10, 'Distance difference');  // Should be very close
});
...
test('1.5 Zero distance', () => {
	...
	const dist = equirectangularDistance(pos, pos);
...
test('3.2 Parallel courses (no collision)', () => {
	...
	const currentDist = equirectangularDistance(vessel1.position, vessel2.position);
